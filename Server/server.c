...
void send_ssh(int sockfd, struct Info2 cli_info2, char *buf, struct ssh_Info ssh_info, char *fileName, char *cli_ip, char *srv_path, char *log, char *log_info, char (*arr_log)[MAXLINE], int *logi)
{
	...
	//strcpy(ssh_cmd, "python ssh.py "); //@@@@@@@@@@@  need to delete   @@@@@@@@@@@
	strcpy(ssh_cmd, "./ssh ");
...
void *thread_function(void *data)
{
	...
	for(f=0; f<list_index; f++)
	{
		...
		else if(hash_res == 1) //fail to open hash text file
		{
			close(sockfd);
			return 0;
		}
	...
	if(access("./update.txt",F_OK) < 0) //if no allHashsum.txt? execute hash_srv.py
	{
		int fd;
