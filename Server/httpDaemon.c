...
int main(int argc, char* argv[])
{
	...
	openlog("Http server start", LOG_CONS, LOG_DAEMON);
	...
	while(1)
	{
		...
		strcpy(http_path, def_arr);
		strcat(http_path, "http_srv");

		if(fork() == 0)
			execl(http_path, "./http_srv", "8000", NULL);
	}
	closelog();
