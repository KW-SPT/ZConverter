...
int main(int argc, char **argv)
{
	...
	/*if(argc!=7) //few factors inputed
	{   
		if(argc == 2)
		{
			if(!strncmp(argv[1], "--help", 6))
			{
				printf("Please enter the options as below!\n");
				printf("<ip> <port_num> <version> <protocol> <path to save> <port number of client>\n");
				return 0;
			}
		}
		strcpy(log, get_time(t, tm));
		strcat(log, " || ");
		strcat(log, "[ERROR] : Incorrect number of factors entered!\n");
		fputs(log, fp);
		fputs("============================================================================================================\n", fp);
		fclose(fp);
		return -1;
	}
	else // way to execute -> ./client ip(server) version protocol path portnum
	{  
		strcpy(IP,argv[1]);
		PORT = atoi(argv[2]);
		strcpy(info2.version,argv[3]);
		strcpy(info2.sendType,argv[4]);
		strcpy(info2.filePath,argv[5]);
		info2.port=atoi(argv[6]);

		if(strncmp(info2.sendType,"https",5) && strncmp(info2.sendType,"websocket",9) && strncmp(info2.sendType,"ssh",3))
		{
			printf("choose protocol only in 3 way! - https, websocket, ssh\n");
			strcpy(log, get_time(t, tm));
			strcat(log, " || ");
			strcat(log, "[ERROR] : Incorrect number of factors entered!\n");
			fputs(log, fp);
