...
int cli_connect(char *ip,int PORT, char ver[], char *new_ver){
	...
	if (write(server_sockfd, buf, MAXLINE) <= 0) /* send to server */{
		perror("write error : ");
		return 1;
	}
	memset(buf, 0x00, MAXLINE);
	...
	if(!strncmp(buf, "yes", 3))
	{
 	...
 	{
		memset(buf, 0x00, MAXLINE);
		strcpy(buf, "Please update your Program later!\n");
	}
...
int main() {
	...
	scanf("%s",ver);
	printf("enter the new version to change : ");
	scanf("%s",new_ver);
	char cmd[256];
	strcpy(cmd, "./log ");
	strcat(cmd,ver);
	...
	if (fp != NULL) {
		...
		while (!feof(fp)) {
			...
			port=strtok(NULL," ");
			printf("ip :%s\nport : %s\n",ip,port);
			cli_connect(ip,atoi(port),ver, new_ver);
